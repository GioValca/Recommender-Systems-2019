...
class IALSRecommender(BaseMatrixFactorizationRecommender, Incremental_Training_Early_Stopping):
    ...
    def save_model(self, folder_path, file_name=None):
        path_users = folder_path + file_name + '_user_factors.npz'
        path_items = folder_path + file_name + '_item_factors.npz'
        np.savez_compressed(path_users, self.USER_factors)
        np.savez_compressed(path_items, self.ITEM_factors)

    def load_model(self, folder_path, file_name=None):
        path_users = folder_path + file_name + '_user_factors.npz'
        path_items = folder_path + file_name + '_item_factors.npz'
        user_factors_loaded = np.load(path_users)
        item_factors_loaded = np.load(path_items)
        user_factors = user_factors_loaded['arr_0']
        item_factors = item_factors_loaded['arr_0']
